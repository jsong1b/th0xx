...
def translateInputs(keeb_device):
    with uinput.Device([
        ...
        while True:
            ...
            shield_pressed = L_Bind in active_keys or R_Bind in active_keys
            ...
            if isSublist([MODX_Bind, MODY_Bind], active_keys):
                ...
                if CLEFT_Bind in active_keys:    cstick_target[0] = -65
                elif CRIGHT_Bind in active_keys: cstick_target[0] = 65
                if CUP_Bind in active_keys:      cstick_target[1] = 23
                elif CDOWN_Bind in active_keys:  cstick_target[1] = -23
            else:
                ...
                if CDOWN_Bind and CLEFT_Bind:    cstick_target = [-35, -98]
                elif CDOWN_Bind and CRIGHT_Bind: cstick_target = [35, -98]
            gamepad_device.emit(uinput.ABS_RX, cstick_target[0], syn = False)
            gamepad_device.emit(uinput.ABS_RY, cstick_target[1], syn = False)

            if Z_Bind in active_keys_vals.keys():
                if active_keys_vals[Z_Bind] == 0: gamepad_device.emit(uinput.ABS_HAT1X, 0)
                else:                             gamepad_device.emit(uinput.ABS_HAT1X, 49)
                gamepad_device.emit(uinput.BTN_A, active_keys_vals[Z_Bind])
            ...
            if L_Bind in active_keys_vals.keys(): gamepad_device.emit(uinput.BTN_TL, active_keys_vals[L_Bind])
...
def grayStickTarget(active_keys, graystick_block_reac, shield_pressed, graystick_prev, graystick_start_time):
    ...
    if MODX_Bind in active_keys:
        if diag:
            if MODB_Bind in active_keys:        target = [85 * dir[0], 41 * dir[1]]
            elif shield_pressed:             target = [82 * dir[0], 35 * dir[1]]
            elif CUP_Bind in active_keys:    target = [77 * dir[0], 55 * dir[1]]
            elif CDOWN_Bind in active_keys:  target = [82 * dir[0], 36 * dir[1]]
            elif CLEFT_Bind in active_keys:  target = [84 * dir[0], 50 * dir[1]]
            elif CRIGHT_Bind in active_keys: target = [72 * dir[0], 61 * dir[1]]
            else:                            target = [70 * dir[0], 60 * dir[1]]
        else: target = [70 * dir[0], 60 * dir[1]] # cardinals
    elif MODY_Bind in active_keys:
        if diag:
            if MODB_Bind in active_keys:        target = [28 * dir[0], 85 * dir[1]]
            elif shield_pressed:             target = [51 * dir[0], 82 * dir[1]]
            elif CUP_Bind in active_keys:    target = [55 * dir[0], 77 * dir[1]]
            elif CDOWN_Bind in active_keys:  target = [34 * dir[0], 82 * dir[1]]
            elif CLEFT_Bind in active_keys:  target = [40 * dir[0], 84 * dir[1]]
            elif CRIGHT_Bind in active_keys: target = [62 * dir[0], 72 * dir[1]]
            else:                            target = [28 * dir[0], 58 * dir[1]]
        else:
            target = [35 * dir[0], 70 * dir[1]] # cardinals
    else:
        if diag:
