# for Super Smash Bros. Ult / Project HDR
# idk if I'm using correct values, most of these are just scaled values from 80 -> 100 from the melee scripts
...
SELECT_Bind = evdev.ecodes.KEY_1
R1_Bind = evdev.ecodes.KEY_L
L1_Bind = evdev.ecodes.KEY_O
R2_Bind = evdev.ecodes.KEY_U
L2_Bind = evdev.ecodes.KEY_SEMICOLON
...
def translateInputs(keeb_device):
    with uinput.Device([
        ...
        while True:
            ...
            shield_pressed = L2_Bind in active_keys or R2_Bind in active_keys
            ...
            if isSublist([MODX_Bind, MODY_Bind], active_keys):
                ...
                if CUP_Bind and CLEFT_Bind:      cstick_target = [-127, 59]
                elif CUP_Bind and CRIGHT_Bind:   cstick_target = [127, 59]
                elif CDOWN_Bind and CLEFT_Bind:  cstick_target = [-127, -59]
                elif CDOWN_Bind and CRIGHT_Bind: cstick_target = [127, -59]
            else:
                if CLEFT_Bind in active_keys:    cstick_target[0] = -100
                elif CRIGHT_Bind in active_keys: cstick_target[0] = 100
                if CUP_Bind in active_keys:      cstick_target[1] = 100
                elif CDOWN_Bind in active_keys:  cstick_target[1] = -100

                if CUP_Bind and CLEFT_Bind:      cstick_target = [-42, 68]
                elif CUP_Bind and CRIGHT_Bind:   cstick_target = [42, 68]
                elif CDOWN_Bind and CLEFT_Bind:  cstick_target = [-42, -68]
            ...
            if L1_Bind in active_keys_vals.keys(): gamepad_device.emit(uinput.BTN_TL2, active_keys_vals[L1_Bind])
            if R1_Bind in active_keys_vals.keys(): gamepad_device.emit(uinput.BTN_TR2, active_keys_vals[R1_Bind])
            if L2_Bind in active_keys_vals.keys(): gamepad_device.emit(uinput.BTN_TL, active_keys_vals[L2_Bind])
            if R2_Bind in active_keys_vals.keys(): gamepad_device.emit(uinput.BTN_TR, active_keys_vals[R2_Bind])
            ...
            if SELECT_Bind in active_keys_vals.keys(): gamepad_device.emit(uinput.BTN_SELECT, active_keys_vals[SELECT_Bind])
...
def grayStickTarget(active_keys, graystick_block_reac, shield_pressed, graystick_prev, graystick_start_time):
    ...
    if MODX_Bind in active_keys:
        if diag:
            if shield_pressed:          target = [51 * dir[0], 30 * dir[1]]
            elif A_Bind in active_keys: target = [36 * dir[0], 26 * dir[1]]
            elif B_Bind in active_keys:
                # diag + b, no shield
                if CUP_Bind in active_keys:      target = [67 * dir[0], 39 * dir[1]]
                elif CDOWN_Bind in active_keys:  target = [67 * dir[0], 55 * dir[1]]
                elif CLEFT_Bind in active_keys:  target = [67 * dir[0], 49 * dir[1]]
                elif CRIGHT_Bind in active_keys: target = [67 * dir[0], 35 * dir[1]]
                else:                            target = [67 * dir[0], 44 * dir[1]]
            else:
                # diag, no shield, no b
                if CUP_Bind in active_keys:      target = [53 * dir[0], 31 * dir[1]]
                elif CDOWN_Bind in active_keys:  target = [53 * dir[0], 43 * dir[1]]
                elif CLEFT_Bind in active_keys:  target = [53 * dir[0], 39 * dir[1]]
                elif CRIGHT_Bind in active_keys: target = [72 * dir[0], 61 * dir[1]]
                else:                            target = [58 * dir[0], 25 * dir[1]]
        elif shield_pressed: target = [51 * dir[0], 51 * dir[0]]
        elif A_Bind in active_keys: target = [36 * dir[0], 0]
        else: target = [53 * dir[0], 44 * dir[1]] # cardinals
    elif MODY_Bind in active_keys:
        if diag:
            if shield_pressed:          target = [39 * dir[0], 69 * dir[1]]
            elif A_Bind in active_keys: target = [34 * dir[0], 38 * dir[1]]
            elif B_Bind in active_keys:
                # diag + b, no shield
                if CUP_Bind in active_keys:      target = [39 * dir[0], 67 * dir[1]]
                elif CDOWN_Bind in active_keys:  target = [55 * dir[0], 67 * dir[1]]
                elif CLEFT_Bind in active_keys:  target = [49 * dir[0], 67 * dir[1]]
                elif CRIGHT_Bind in active_keys: target = [35 * dir[0], 67 * dir[1]]
                else:                            target = [44 * dir[0], 67 * dir[1]]
            else:
                # diag, no shield, no b
                if CUP_Bind in active_keys:      target = [31 * dir[0], 53 * dir[1]]
                elif CDOWN_Bind in active_keys:  target = [43 * dir[0], 53 * dir[1]]
                elif CLEFT_Bind in active_keys:  target = [49 * dir[0], 53 * dir[1]]
                elif CRIGHT_Bind in active_keys: target = [28 * dir[0], 53 * dir[1]]
                else:                            target = [35 * dir[0], 53 * dir[1]]
        elif A_Bind in active_keys: target = [36 * dir[0], 36 * dir[0]]
        else: target = [41 * dir[0], 53 * dir[1]] # cardinals
    else:
        if diag:
            if dir[1] == 1:    target = [83 * dir[0], 93]
            elif dir[1] == -1: target = [100 * dir[0], 100 * dir[1]]
        else: target = list(map(lambda x: x * 100, dir))
    ...
    def isAdjacent(coord1, coord2):
        ...
        if (checkSDI(coord1) == "card" and checkSDI(coord2) == "diag" and (
            (coord1[0] >= 70 and coord2[0] > 0) or
            (coord1[0] <= -70 and coord2[0] < 0) or
            (coord1[1] >= 70 and coord2[1] > 0) or
            (coord1[1] <= -70 and coord2[1] < 0))): return True
        # card -> diag
        if (checkSDI(coord1) == "diag" and checkSDI(coord2) == "card" and (
            (coord2[0] >= 70 and coord1[0] > 0) or
            (coord2[0] <= -70 and coord1[0] < 0) or
            (coord2[1] >= 70 and coord1[1] > 0) or
            (coord2[1] <= -70 and coord1[1] < 0))): return True
        # diag -> diag
    ...
    if len(graystick_prev) >= 2:
        ...
        # dash back nerfs
        if (abs(graystick_prev[0][0][0]) >= 80 and abs(graystick_prev[1][0][0] >= 80) and abs(graystick_prev[0][0][0] + graystick_prev[1][0][0]) >= 128 and graystick_prev[0][1] - graystick_prev[1][1] <= 0.25):
            if (target[1] >= 29 and target[1] <= 65): return(reac, [dir[0] * 74, 67], time.time())
            if (target[1] >= -65 and target[1] <= -29):
                angle = math.atan2(target[1], target[0])
                return(reac, [math.ceil(100 * math.cos(angle)), math.ceil(100 * math.sin(angle))], time.time())

    if len(graystick_prev) >= 1:
        # conditionally inaccessible coordinates
        if (graystick_prev[0][0][1] < 0 and target[1] >= 29 and target[1] <= 65): return (reac, [dir[0] * 74, 67], time.time())
...
def grayStickTravelTime(fuzz, cur, target, prev, lerp_start, start_time):
    ...
    if tar_sq <= 9663: travel_time = 12
    elif tar_sq >= 9664 and tar_sq <= 10714: travel_time = 6
    elif tar_sq >= 10715 and tar_sq <= 14029: travel_time = 7
    elif tar_sq >= 14030: travel_time = 8
